...
- When applying transitions-dev, expects the skill’s CSS and interaction wiring faithfully (e.g. avatar group hover with visible lift, scale, and neighbor falloff), not a simplified substitute.
- When iterating on UI in this repo, wants the Vite dev server running (`npm run dev`) so changes live-update in the browser at `http://localhost:5173/` via HMR, not the GitHub Pages build; `vite/dev-reload-on-data.ts` full-reloads on `src/data/` edits so profile/content changes show without a manual refresh.
- Keeps music/artist social profiles private (not listed publicly); personal dual Instagram and TikTok accounts use the SocialGrid profile chooser instead.

## Learned Workspace Facts

- The `ash` project is a React + TypeScript personal link site built with Vite; `ErrorBoundary` wraps the app in `main.tsx`; tests use Vitest and Testing Library, and ESLint is used for linting (with `react-refresh/only-export-components` disabled for `src/components/ui/**`).
- Site content is centralized in `src/data/profile.ts` (`links`, `extras`, etc.); the main layout composes Hero, SocialGrid, ExtrasSection (“More”), ProjectsSection, ContactSection, and Footer. Dual personal Instagram (`@itsdavidig` + `@calissick`) and TikTok (`@itsash583` + `@caldidsumshi`) use the SocialGrid profile chooser (`artistUrl` holds the second personal URL); Instagram is not pinned/primary and sorts with other links. Cursor and OpenCode Go referrals live in `profile.extras`, not the social grid.
...
- Live site: `https://itsasheruwu.github.io/ash` (GitHub repo `itsasheruwu/ash`). `.github/workflows/pages.yml` deploys on push to `main` via GitHub Actions. Vite inlines `VITE_*` at build time; set `VITE_SPOTIFY_STATUS_URL` and `VITE_INSTAGRAM_AVATAR_URL` as Actions repo secrets so production matches local.
- `SpotifyStatusPill` can derive track title colors from album art via `src/lib/extractVibrantColorsFromImageUrl.ts`; on mobile (≤480px) `useSpotifyPillDock` tucks the pill to the left edge (drag or tap to reveal).
- Developer portfolio block: `ProjectsSection` below the links card (heading “Development Projects”); project data in `src/data/projects.ts` (includes `video-frame-post-picker` with icon under `public/projects/`), languages and collapsible dev tools in `src/data/developer.ts` (Codex Pro Lite, Cursor Pro, OpenCode Go, Claude Code with usage-based pricing note); overlapping icon chips in `t-avatar-group` (taglines only in modal); uses transitions-dev (`t-panel-slide`, `t-stagger`, `t-avatar`, `t-tt`, `t-modal`). Tool icons: `CodexIcon`, `CursorIcon`, `OpenCodeIcon` under `src/components/icons/`, Claude via `SiClaude`.
